...
function drawFirstPage() {
  ...
  let newHeight = imgHeight * 1.3; // Aumentamos el tamaño en un 30%
  ...
  text("JOSEP MARTÍNEZ PEDRO", width / 2, 100); // Ajustamos la posición en la parte superior del lienzo
}
...
function mouseMoved() {
  ...
  if (mouseX > imgLeft && mouseX < imgRight && mouseY > imgTop && mouseY < imgBottom) {
    // Aumentamos la opacidad del texto progresivamente hasta 255 (más rápido)
    if (textOpacity < 255) {
      textOpacity += 30;  // Incremento más rápido de la opacidad
    }
  } else {
    // Si el ratón sale de la imagen, el texto vuelve a ser invisible
    if (textOpacity > 0) {
      textOpacity -= 30;  // Decrecimiento más rápido de la opacidad
    }
  }
}
